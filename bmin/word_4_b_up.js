...
function solution(record, word_dict) {
  ...
  while (true) {
    let searchWord = record.slice(0, word_dict[i].length);

    if (searchWord != word_dict[i]) {
      break;
    }
    searchCountList[i] = 1;
    let searchRecord = record.substr(word_dict[i].length);

    for (j = 1; j < word_dict.length; j++) {
      searchWord = searchRecord.slice(0, word_dict[j].length);

      if (searchWord == word_dict[j]) {
        searchRecord = searchRecord.substr(word_dict[j].length);
        searchCountList[i]++;
      }
    }
    i++;
  ...
  return Math.max(...searchCountList);
}

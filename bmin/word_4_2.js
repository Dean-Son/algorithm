...
function solution(s, wordDict) {
  const dp = new Array(s.length + 1).fill(0); // DP 테이블 초기화
  const trie = {};
  ...
  for (let i = 1; i <= s.length; i++) {
    for (let j = 0; j < i; j++) {
      ...
      if (found && node.isEnd) {
        console.log(word, i, found, JSON.stringify(node));
        dp[i] = Math.max(dp[i], dp[j] + 1);
      }
    }
  }
  console.log(`dp === ${dp}`);
  return dp[s.length] - 1; // 빈 문자열을 제외한 실제 단어 개수
}
